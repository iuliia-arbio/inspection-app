...
Files taken verbatim, unmodified:
- `speedtest.js` — main client interface (`window.Speedtest`)
- `speedtest_worker.js` — background worker that runs the actual transfer test
...
- any modifications *we* make to these files would need to be disclosed under the same
  license. We have not modified them. If a future change requires patching these files,
  document the change here and keep the LGPL notice.
